...
func InjectQueryDBMiddleware(controller interface{}) fiber.Handler {
	return func(c *fiber.Ctx) error {
		dbName, ok := c.Locals("unit").(string)
		if !ok || dbName == "" {
			return fiber.NewError(fiber.StatusInternalServerError, "database name not found in context")
		...
		// Pakai GetDBConnection yang sama
		queryDB, err := GetDBConnection(dbName)
		if err != nil {
			return fiber.NewError(fiber.StatusInternalServerError, "error connecting to query database")
		}
		...
		elem := val.Elem()
		dbField := elem.FieldByName("QueryDB")
		if !dbField.IsValid() || !dbField.CanSet() {
			return fiber.NewError(fiber.StatusInternalServerError, "QueryDB field not found or cannot be set in controller")
		}

		if dbField.Type() != reflect.TypeOf((*gorm.DB)(nil)) {
			return fiber.NewError(fiber.StatusInternalServerError, "QueryDB field has wrong type")
		}
