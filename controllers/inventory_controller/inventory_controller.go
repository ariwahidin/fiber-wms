...
func (c *InventoryController) UpdateInventoryPolicy(ctx *fiber.Ctx) error {
	...
	policy.InboundCanInputSerial = updateData.InboundCanInputSerial
	policy.AllocationLotByOrder = updateData.AllocationLotByOrder
	policy.AllocationLocationByOrder = updateData.AllocationLocationByOrder
	policy.AllocationCartonByOrder = updateData.AllocationCartonByOrder
	policy.AllocationCaseByOrder = updateData.AllocationCaseByOrder
	policy.AllocationSerialByOrder = updateData.AllocationSerialByOrder
...
const (
	maxSQLParams        = 2100
	movementColumnCount = 30 // jumlah kolom yang di-insert ke InventoryMovement
)

func safeBatchSize(columnCount int) int {
	size := int(float64(maxSQLParams) / float64(columnCount) * 0.8)
	if size < 1 {
		size = 1
	}
	return size
...
func bulkUpdateLotNumberWithHistory(
	...
	for i := 0; i < len(updates); i += chunkSize {
		...
		movementBatchSize := safeBatchSize(movementColumnCount)
		if err := tx.CreateInBatches(&movements, movementBatchSize).Error; err != nil {
			return totalAffected, err
		}
