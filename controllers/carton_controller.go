...
func (c *MasterCartonController) Create(ctx *fiber.Ctx) error {
	...
	// Validate unique combination of Length, Width, Height, CartonCode
	var existingCarton models.MasterCarton
	if err := c.DB.Where("length = ? AND width = ? AND height = ? AND carton_code = ?",
		carton.Length, carton.Width, carton.Height, carton.CartonCode).First(&existingCarton).Error; err == nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"success": false,
			"message": "Master Carton with the same Length, Width, Height, and Carton Code already exists",
		})
	}
...
func (c *MasterCartonController) Update(ctx *fiber.Ctx) error {
	...
	var existingCarton models.MasterCarton
	if err := c.DB.Where("length = ? AND width = ? AND height = ? AND carton_code = ? AND id != ?",
		carton.Length, carton.Width, carton.Height, carton.CartonCode, id).First(&existingCarton).Error; err == nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"success": false,
			"message": "Master Carton with the same Length, Width, Height, and Carton Code already exists",
		})
