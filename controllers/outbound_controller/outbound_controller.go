...
func (c *OutboundController) CreateOutboundFromEcommerceExcel(ctx *fiber.Ctx) error {
	...
	if platform == "" {
		return ctx.Status(fiber.StatusBadRequest).JSON(EcommerceUploadResponse{
			Success: false,
			Message: "Platform is required (SHOPEE, TOKOPEDIA, LAZADA, TIKTOK)",
		})
	}
	...
	if !ok {
		return ctx.Status(fiber.StatusBadRequest).JSON(EcommerceUploadResponse{
			Success: false,
			Message: "Unsupported platform: " + platform + ". Supported: SHOPEE, TOKOPEDIA, LAZADA, TIKTOK",
		})
	}
	...
	// 7. Validate header columns
	if err := c.validateEcommerceExcelHeader(rows[0]); err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(EcommerceUploadResponse{
			Success: false,
			Message: "Invalid Excel template: " + err.Error(),
			ValidationErrors: []ValidationError{
				{Field: "Header", Message: err.Error(), Row: 1},
			},
		})
	...
	// 8. Parse rows
	orderRows, validationErrors := c.parseEcommerceExcelRows(rows)
	if len(validationErrors) > 0 {
		return ctx.Status(fiber.StatusBadRequest).JSON(EcommerceUploadResponse{
			Success:          false,
			Message:          fmt.Sprintf("Validation failed with %d errors", len(validationErrors)),
	...
	// 9. Group rows by OrderNumber → map[orderNumber][]EcommerceOrderRow
	...
	// 12. Validate platform master data (customer, transporter, warehouse, owner, inventory policy)
	var inventoryPolicy models.InventoryPolicy
	if err := tx.Where("owner_code = ?", platformConfig.OwnerCode).First(&inventoryPolicy).Error; err != nil {
		tx.Rollback()
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ctx.Status(fiber.StatusNotFound).JSON(EcommerceUploadResponse{
				Success: false,
				Message: "Inventory Policy not found for owner: " + platformConfig.OwnerCode,
			})
		}
		return ctx.Status(fiber.StatusInternalServerError).JSON(EcommerceUploadResponse{
			Success: false,
	...
	if err := tx.First(&customerTo, "customer_code = ?", customerCode).Error; err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ctx.Status(fiber.StatusNotFound).JSON(EcommerceUploadResponse{
				Success: false,
				Message: "Delivery-to customer not found for platform: " + platform,
			})
		}
		return ctx.Status(fiber.StatusInternalServerError).JSON(EcommerceUploadResponse{
			Success: false,
			Message: "Failed to validate delivery-to: " + err.Error(),
		})
	}
	...
	if err := tx.First(&warehouse, "code = ?", platformConfig.WhsCode).Error; err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ctx.Status(fiber.StatusNotFound).JSON(EcommerceUploadResponse{
				Success: false,
				Message: "Warehouse not found: " + platformConfig.WhsCode,
			})
		}
		return ctx.Status(fiber.StatusInternalServerError).JSON(EcommerceUploadResponse{
			Success: false,
	...
	// 13. Validate all SKUs exist in products table
	skuValidationErrors := c.validateEcommerceSKUs(tx, orderRows)
	if len(skuValidationErrors) > 0 {
		tx.Rollback()
		return ctx.Status(fiber.StatusBadRequest).JSON(EcommerceUploadResponse{
			Success:          false,
			Message:          "SKU validation failed",
			ValidationErrors: skuValidationErrors,
			TotalRows:        len(orderRows),
		})
	}

	// 14. Create one outbound per order number
	repositories := repositories.NewOutboundRepository(tx)
	var outboundNumbers []string
	totalSuccessItems := 0

	// Process in stable order (sort order numbers)
	...
	for _, orderNumber := range orderNumbers {
		...
		// Use date from first row of this order
		...
		// Create outbound details for each item in this order
		...
		if err := helpers.InsertTransactionHistory(tx, outboundNo, "open", "OUTBOUND",
			fmt.Sprintf("Created from %s ecommerce upload - Order: %s", platform, orderNumber), userID); err != nil {
			log.Printf("Warning: Failed to insert transaction history for %s: %v", outboundNo, err)
		}
	...
	return ctx.Status(fiber.StatusOK).JSON(EcommerceUploadResponse{
		Success:         true,
		Message:         fmt.Sprintf("Successfully created %d outbound(s) with %d items from %s", len(outboundNumbers), totalSuccessItems, platform),
		TotalRows:       len(orderRows),
...
// -----------------------------------------------------------------------
// Helper: validateEcommerceExcelHeader
// Expected columns: No | Date | Platform | Order Number | AWB Number | Product Name | Qty | SKU
// -----------------------------------------------------------------------
func (c *OutboundController) validateEcommerceExcelHeader(headerRow []string) error {
	expected := []string{"No", "Date", "Platform", "Order Number", "AWB Number", "Product Name", "Qty", "SKU"}
	if len(headerRow) < len(expected) {
		return fmt.Errorf("expected %d columns, got %d", len(expected), len(headerRow))
	}
	for i, col := range expected {
		actual := strings.TrimSpace(headerRow[i])
		if !strings.EqualFold(actual, col) {
			return fmt.Errorf("column %d: expected '%s', got '%s'", i+1, col, actual)
		}
	}
	return nil
}
...
// -----------------------------------------------------------------------
// Helper: parseEcommerceExcelRows
// -----------------------------------------------------------------------
func (c *OutboundController) parseEcommerceExcelRows(rows [][]string) ([]EcommerceOrderRow, []ValidationError) {
	...
	for i := 1; i < len(rows); i++ {
		...
		// Skip row if Order Number is empty
		orderNumber := strings.TrimSpace(getCell(row, 3))
		if orderNumber == "" {
			continue
		}

		// SKU is required
		skuRaw := strings.TrimSpace(getCell(row, 7))
		if skuRaw == "" {
		...
		// Qty
		qtyStr := strings.TrimSpace(getCell(row, 6))
		if qtyStr == "" {
			errs = append(errs, ValidationError{
				Field:   "Qty",
		...
		// Date — accept both datetime string and date-only string
		dateRaw := strings.TrimSpace(getCell(row, 1))
		parsedDate := parseEcommerceDate(dateRaw)
		if parsedDate == "" {
			errs = append(errs, ValidationError{
				Field:   "Date",
		...
		// Platform from Excel (informational — we use the form platform)
		platformFromExcel := strings.TrimSpace(getCell(row, 2))

		item := EcommerceOrderRow{
			Date:        parsedDate,
			Platform:    platformFromExcel,
			OrderNumber: orderNumber,
			AWBNumber:   strings.TrimSpace(getCell(row, 4)),
			ProductName: strings.TrimSpace(getCell(row, 5)),
			Qty:         qty,
			SKU:         skuRaw,
			Row:         rowNum,
		}
...
// -----------------------------------------------------------------------
// Helper: parseEcommerceDate
// Accepts: "2026-02-19 19:01:00", "2026-02-19", "19/02/2026", etc.
// Returns: "YYYY-MM-DD" string or "" on failure
// -----------------------------------------------------------------------
func parseEcommerceDate(raw string) string {
	if raw == "" {
		return ""
	}
	formats := []string{
		...
		// Excel short year formats (e.g. "2/19/26 19:01" from Excel)
		"1/2/06 15:04",
		"1/2/06 3:04 PM",
		"01/02/06 15:04",
		// Date only
...
// -----------------------------------------------------------------------
// Helper: groupEcommerceRowsByOrderNumber
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Helper: validateEcommerceSKUs
// -----------------------------------------------------------------------
func (c *OutboundController) validateEcommerceSKUs(tx *gorm.DB, rows []EcommerceOrderRow) []ValidationError {
	var errs []ValidationError

	// Deduplicate SKUs for efficient DB queries
	skuSet := make(map[string]bool)
	skuRowMap := make(map[string]int) // SKU → first row number
	for _, r := range rows {
		if !skuSet[r.SKU] {
			skuSet[r.SKU] = true
	...
	for sku, rowNum := range skuRowMap {
		...
		if err := tx.First(&product, "item_code = ?", sku).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				errs = append(errs, ValidationError{
					Field:   "SKU",
					Message: "Product not found with SKU/item_code: " + sku,
					Row:     rowNum,
				})
			} else {
				errs = append(errs, ValidationError{
...
// ======================================================================
// END PROCESS UPLOAD OUTBOUND FROM ECOMMERCE EXCEL
// ======================================================================
