...
func sendViaGoogleSheets(
	...
	// ─── Tentukan mode berdasarkan apakah dipanggil dari scheduler ───────────
	isScheduled := false
	if v, ok := eventData["scheduled"]; ok {
		if b, ok := v.(bool); ok {
			isScheduled = b
		}
	}
	...
	if isScheduled {
		return handleScheduledMode(srv, conn, sheetName, headers, rows)
	}

	// ─── Realtime mode — upsert by order_no ──────────────────────────────────
	return handleRealtimeMode(srv, conn, sheetName, headers, rows)
}

// ─── Scheduled Mode ───────────────────────────────────────────────────────────

func handleScheduledMode(
	srv *sheets.Service,
	conn integration.IntegrationConnection,
	sheetName string,
	headers []string,
	...
	scheduleMode := conn.ScheduleMode
	if scheduleMode == "" {
		scheduleMode = "overwrite" // default untuk scheduled
	}

	switch scheduleMode {
	case "overwrite":
		// Hapus semua isi sheet, tulis ulang dari awal
		return overwriteSheet(srv, conn.SpreadsheetID, sheetName, headers, rows, conn.HeaderRow)

	case "append":
		// Tambah terus di bawah data yang sudah ada
		return appendToSheet(srv, conn.SpreadsheetID, sheetName, headers, rows, conn.HeaderRow)

	case "new_sheet":
		// Buat tab baru per tanggal: "2026-02-25"
		newSheetName := time.Now().Format("2006-01-02")

		// Buat tab baru kalau belum ada
		if err := ensureSheetExists(srv, conn.SpreadsheetID, newSheetName); err != nil {
			return fmt.Errorf("gagal buat tab sheet baru: %w", err)
		}

		// Tulis ke tab baru
		return appendToSheet(srv, conn.SpreadsheetID, newSheetName, headers, rows, conn.HeaderRow)

	default:
		return fmt.Errorf("schedule_mode tidak dikenal: '%s' (gunakan: overwrite/append/new_sheet)", scheduleMode)
	}
}

// ─── Realtime Mode ────────────────────────────────────────────────────────────

func handleRealtimeMode(
	srv *sheets.Service,
	conn integration.IntegrationConnection,
	sheetName string,
	headers []string,
	...
	orderNo := extractOrderNo(conn, rows, headers)
	if orderNo == "" {
		// Tidak ada order_no → append saja
		return appendToSheet(srv, conn.SpreadsheetID, sheetName, headers, rows, conn.HeaderRow)
	}
...
// extractOrderNo ambil order_no dari eventData atau dari baris pertama hasil query
func extractOrderNo(conn integration.IntegrationConnection, rows [][]interface{}, headers []string) string {
	// Cari dari headers hasil query — cocokkan dengan KeyColumn atau knownKeyColumns
	target := strings.TrimSpace(strings.ToLower(conn.KeyColumn))

	for i, h := range headers {
		colName := strings.TrimSpace(strings.ToLower(h))

		matched := false
		if target != "" {
			matched = colName == target
		} else {
			for _, knownKey := range knownKeyColumns {
				if colName == knownKey {
					matched = true
		...
		if matched && len(rows) > 0 && i < len(rows[0]) {
			val := strings.TrimSpace(fmt.Sprintf("%v", rows[0][i]))
			if val != "" && val != "<nil>" {
				return val
			}
		}
...
func detectKeyColumn(
	...
	// Priority 1: KeyColumn dari config
	if keyColumn != "" {
		target := strings.TrimSpace(strings.ToLower(keyColumn))
		for i, cell := range headers {
			colName := strings.TrimSpace(strings.ToLower(fmt.Sprintf("%v", cell)))
		...
		fmt.Printf("[GoogleSheets] Warning: KeyColumn '%s' tidak ditemukan, fallback auto-detect\n", keyColumn)
	}
...
func findRowsByKey(
	...
	for i, row := range resp.Values {
		if len(row) > 0 {
			cellVal := strings.TrimSpace(fmt.Sprintf("%v", row[0]))
			if cellVal == keyValue {
				matchedRows = append(matchedRows, i+1)
			}
		}
	}
...
// ensureSheetExists buat tab sheet baru kalau belum ada
func ensureSheetExists(srv *sheets.Service, spreadsheetID string, sheetName string) error {
	spreadsheet, err := srv.Spreadsheets.Get(spreadsheetID).Do()
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("gagal buat sheet '%s': %w", sheetName, err)
	}

	fmt.Printf("[GoogleSheets] Tab baru dibuat: '%s'\n", sheetName)
	return nil
}
...
func fetchFromEvent(eventData map[string]interface{}) ([]string, [][]interface{}) {
	var headers []string
	for k := range eventData {
		if k != "year" && k != "date" && k != "datetime" && k != "scheduled" && k != "trigger_by" {
			headers = append(headers, k)
		}
	}
